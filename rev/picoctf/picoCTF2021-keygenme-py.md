...
# overview
<img width="446" height="239" alt="image" src="https://github.com/user-attachments/assets/2453ca11-ac1c-4148-b59e-fe6111c1a951" />

sesuai namanya, untuk mendapatkan flag di di challenge ini kita harus membuat keygen (key generator) yang dimanaa key nya adalah flag itu sendiri. untuk mendapatkan keynya 
kita harus menganalisis program yang membandingkan input lisensi kita dengan hash sha256 yang ada di program. file yang dibagi juga adalah source code python jadi akan sedikit lebih
mudah dipahami daripada bahasa hasil decompiler pada chall reverse engineering umum

# recon  

**note** : recon ini membahas bagaimana caraku mencari semua informasi yang dibutuhkan untuk mencari keygennya dari nol. jika ingin mencari skrip keygen bisa skip ke bagian `exploit`  


pertama download dulu file challengenya : `keygenme-trial.py`  lalu coba kujalankan.
```
┌[rotalactf]-[LAPTOP-6QMID52F]-(keygenme)
...
Enter your license key: dsds

Key is NOT VALID. Check your data entry.
```

sepertinya aku tidak tahu bagaimana cara program ini bekerja.

jadi aku coba membaca sourcenya menggunakan vs codium. berikut isi program python tersebut :
```Python
#============================================================================#
#============================ARCANE CALCULATOR===============================#
#============================================================================#
...
karena cukup banyak fungsi yang dideklarasikan disini. jadi aku coba cari fungsi utama/ fungsi main program nya terlebih dahulu.

awal program akan mengeksekusi fungsi `ui_flow()` yang nantinya disusul fungsi `intro_trial()` dan `menu_trial()`
```Python
def ui_flow():
    intro_trial()
    while arcane_loop_trial:
        menu_trial()
```
diantara dua fungsi tersebut, menurutku mainnya adalah `menu_trial()` karena intro_trial() hanya melakukan print teks intro. berikut isi program menu :
...
nah `estimate_burn()`, `locked_estimate_vector()`,dan `enter_license()`. dari ketiga fungsi tersebut yang paling menarik adalah enter_license() . karena fungsi ini akan meminta input
user, lalu di bandingkan dengan hasil hash sha256 dari username_trial 
...
def check_key(key, username_trial):
    ...
    if len(key) != len(key_full_template_trial):
        ...
        return True
```
tujuan kita adalah mendapatkan key yang benar agar bisa bypass check_key tersebut. jika melihat ke barisan atas kode
terdapat potongan key yang merupakan flag kita tapi belum lengkap
```
username_trial = "BENNETT"
bUsername_trial = b"BENNETT"
...
key kita adalah variabe key_full_template_trial . dan kita harus mencari key_part_dynamic_trial1 . yang jumlahnya 8 karakter ini. , nah jika melihat ke fungsi check_key(). 
ternyata ada pengecekan dynamic key yang dilakukan 8 kali juga. sepertinya kita bisa memanfaatkan fungsi itu untuk mencari keynya
...
if key[i] != hashlib.sha256(username_trial).hexdigest()[4]:
            return False
```
pada bagian dynamic key, jika index i tidak sama dengan hasil hash username trial index ke 4 , maka program mengembalikan nilai false.(key salah). aku sudah mengumpulkan ke delapan 

index yang digunakan : `[4,5,3,6,2,7,1,8]`   
lalu username_trial : `b"BENNETT"`
dan rumus key lengkap = `"picoCTF{1n_7h3_kk3y_of_"` + `dynamic_key` + `}`

# exploit
untuk membuat keygen kita perlu mencari nilai dari dynamic key. nah kita bisa mengecek nilai dynamic key tersebut dengan hasil hash username_trial + index yang digunakan. untuk membuatnya
butuh skrip python sederhana :
```Python
import hashlib
from pwn import *
...
keyindex = [4,5,3,6,2,7,1,8]

key = []
for index in keyindex:
    key += {enc[index]}

key_full_template_trial = key_part_static1_trial + "".join(key) + key_part_static2_trial

def KeyValid():
    context.log_level = 'error'
    io = process(['python3' ,'keygenme-trial.py'])
    io.sendline(b'c')
    io.recvuntil(b'Enter your license key: ')
    io.sendline(key_full_template_trial.encode())
    data = io.recv().strip().decode()
    io.close()
    if 'Exiting trial version...' in data:
        print(f'{'License key valid' :<20} : {key_full_template_trial}')
        return True
    else:
        print('key not valid')
        return False
# mengirim dan memeriksa apakah full key valid

if KeyValid():
    print(f'{'SHA256 ' :<20} : {enc}')
    print(f'{'index key ' :<20} : {keyindex}')
    print(f'{'exctracted key ' :<20} : {key}')
```

result : 
<img width="684" height="111" alt="image" src="https://github.com/user-attachments/assets/66bbbcc0-1d35-4df5-af12-33a0b5f964e3" />

done
flag : `picoCTF{1n_7h3_kk3y_of_...........}`
# conclusion
untuk dapat mengerjakan ini membutuhkan sedikit banyak pemahaman mengenai pemrograman python agar dapat membaca alur program dan apa yang program tersebut lakukan kepada input kita.
